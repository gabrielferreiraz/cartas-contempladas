...
export async function fetchCSV(url: string): Promise<FetchResult> {
  ...
  bustUrl.searchParams.set('_t', `${Date.now()}-${Math.random().toString(36).slice(2)}`);

  let response: Response;
  try {
    response = await fetch(bustUrl.toString(), {
  ...
  // Diagnóstico: se Age > 0 ou X-Cache: HIT, o Google está servindo da cache
  const age = response.headers.get('age');
  const xCache = response.headers.get('x-cache');
  const resCC = response.headers.get('cache-control');
  logger.info('resposta_headers', { age, xCache, cacheControl: resCC });

  const rawContent = await response.text();

  // Normaliza: remove BOM UTF-8 + padroniza quebras de linha (CRLF/CR → LF)
